...
class workspace:
    ...
    def __init__(self, workspace_name, params, role):
        ...
        if self.role == 'train_workspace':
            ...
            self.read_supporting_sets(trainfile)   #Updates supporting_sets, Labelwise sentences, length=2
            self.target_sets.append(self.read_sentences(trainfile))  #All sentences
            self.target_sets_files.append(trainfile)  #Train file, length=1
            self.labels_in_train = range(0, len(self.supporting_sets))
            self.train_intent2ids_list = dict([(l, []) for l in self.labels_in_train]) #For every label, list of idx of sentences
